 ...
 */
package frasc.zstd.fse;

import static frasc.zstd.Util.checkArgument;
import static frasc.zstd.BitInputStream.peekBits;
...
public class FiniteStateEntropy {
    ...
    public static int decompress(FiniteStateEntropy.Table table, final Object inputBase, final long inputAddress,
            final long inputLimit, byte[] outputBuffer) {
    ...
    public static int compress(Object outputBase, long outputAddress, int outputSize, byte[] input, int inputSize,
    ...
    public static int compress(Object outputBase, long outputAddress, int outputSize, Object inputBase,
            long inputAddress, int inputSize, FseCompressionTable table) {
    ...
    public static int optimalTableLog(int maxTableLog, int inputSize, int maxSymbol) {
        ...
        result = Math.min(result, Util.highestBit((inputSize - 1)) - 2); // we may be able to reduce accuracy if input
                                                                         // is small
    ...
    public static int normalizeCounts(short[] normalizedCounts, int tableLog, int[] counts, int total, int maxSymbol) {
        ...
        if (-stillToDistribute >= (normalizedCounts[largest] >>> 1)) {
            ...
            // TODO size_t const errorCode = FSE_normalizeM2(normalizedCounter, tableLog,
    ...
    private static int normalizeCounts2(short[] normalizedCounts, int tableLog, int[] counts, int total,
            int maxSymbol) {
        int distributed = 0;

        int lowThreshold = total >>> tableLog; // minimum count below which frequency in the normalized table is "too
                                               // small" (~ < 1)
        int lowOne = (total * 3) >>> (tableLog + 1); // 1.5 * lowThreshold. If count in (lowThreshold, lowOne] => assign
                                                     // frequency 1
    ...
    public static int writeNormalizedCounts(Object outputBase, long outputAddress, int outputSize,
            short[] normalizedCounts, int maxSymbol, int tableLog) {
        ...
        while (remaining > 1) {
            if (previousIs0) {
                ...
                // encode in batches if 8 repeat sequences in one shot (representing 24 symbols
                // total)
