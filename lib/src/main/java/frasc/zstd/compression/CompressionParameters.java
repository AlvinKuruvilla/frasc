...
import static frasc.zstd.Constants.MIN_WINDOW_LOG;
import static frasc.zstd.Constants.MAX_WINDOW_LOG;
import static frasc.zstd.Util.highestBit;
import static frasc.zstd.Util.cycleLog;
...
public class CompressionParameters {
    ...
    private final int windowLog; // largest match distance : larger == more compression, more memory needed
                                 // during decompression
    private final int chainLog; // fully searched segment : larger == more compression, slower, more memory
                                // (useless for fast)
    ...
    private final int searchLength; // match length searched : larger == faster decompression, sometimes less
                                    // compression
    private final int targetLength; // acceptable match size for optimal parser (only) : larger == more compression,
                                    // slower
    ...
    public enum Strategy {
        ...
        // YC: fast is a "single probe" strategy : at every position, we attempt to find
        // a match, and give up if we don't find any. similar to lz4.
        FAST(BlockCompressor.UNSUPPORTED),

        // YC: double_fast is a 2 attempts strategies. They are not symmetrical by the
        // way. One attempt is "normal" while the second one looks for "long matches".
        // It was
        // empirically found that this was the best trade off. As can be guessed, it's
        // slower than single-attempt, but find more and better matches, so compresses
        // better.
        DFAST(new DoubleFastBlockCompressor()),

        // YC: greedy uses a hash chain strategy. Every position is hashed, and all
        // positions with same hash are chained. The algorithm goes through all
        // candidates. There are
        // diminishing returns in going deeper and deeper, so after a nb of attempts
        // (which can be selected), it abandons the search. The best (longest) match
        // wins. If there is
        // one winner, it's immediately encoded.
        GREEDY(BlockCompressor.UNSUPPORTED),

        // YC: lazy will do something similar to greedy, but will not encode
        // immediately. It will search again at next position, in case it would find
        // something better.
        // It's actually fairly common to have a small match at position p hiding a more
        // worthy one at position p+1. This obviously increases the search workload. But
        // the
        // resulting compressed stream generally contains larger matches, hence
        // compresses better.
        LAZY(BlockCompressor.UNSUPPORTED),

        // YC: lazy2 is same as lazy, but deeper. It will search at P, P+1 and then P+2
        // in case it would find something even better. More workload. Better matches.
        LAZY2(BlockCompressor.UNSUPPORTED),

        // YC: btlazy2 is like lazy2, but trades the hash chain for a binary tree. This
        // becomes necessary, as the nb of attempts becomes prohibitively expensive. The
        // binary tree
        // complexity increases with log of search depth, instead of proportionally with
        // search depth. So searching deeper in history quickly becomes the dominant
        // operation.
        // btlazy2 cuts into that. But it costs 2x more memory. It's also relatively
        // "slow", even when trying to cut its parameters to make it perform faster. So
        // it's really
        // a high compression strategy.
        BTLAZY2(BlockCompressor.UNSUPPORTED),

        // YC: btopt is, well, a hell of lot more complex.
        // It will compute and find multiple matches per position, will dynamically
        // compare every path from point P to P+N, reverse the graph to find cheapest
        // path, iterate on
        // batches of overlapping matches, etc. It's much more expensive. But the
        // compression ratio is also much better.
        BTOPT(BlockCompressor.UNSUPPORTED),

        // YC: btultra is about the same, but doesn't cut as many corners (btopt
        // "abandons" more quickly unpromising little gains). Slower, stronger.
    ...
    public CompressionParameters(int windowLog, int chainLog, int hashLog, int searchLog, int searchLength,
            int targetLength, Strategy strategy) {
