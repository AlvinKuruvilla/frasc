...
import frasc.zstd.Decompressor;
...
public class ZstdDecompressor implements Decompressor {
    ...
    @Override
    public int decompress(byte[] input, int inputOffset, int inputLength, byte[] output, int outputOffset,
            int maxOutputLength)
    ...
    public void decompress(ByteBuffer inputBuffer, ByteBuffer outputBuffer)
        ...
        // HACK: Assure JVM does not collect Slice wrappers while decompressing, since
        // the
        ...
        synchronized (input) {
            synchronized (output) {
                int written = new ZstdFrameDecompressor().decompress(inputBase, inputAddress, inputLimit, outputBase,
