...
import frasc.zstd.compression.CompressionParameters;
...
public class SequenceEncoder {
    ...
    private static final FseCompressionTable DEFAULT_LITERAL_LENGTHS_TABLE = FseCompressionTable.newInstance(
            DEFAULT_LITERAL_LENGTH_NORMALIZED_COUNTS, Constants.MAX_LITERALS_LENGTH_SYMBOL,
    ...
    private static final FseCompressionTable DEFAULT_MATCH_LENGTHS_TABLE = FseCompressionTable.newInstance(
            DEFAULT_MATCH_LENGTH_NORMALIZED_COUNTS, Constants.MAX_MATCH_LENGTH_SYMBOL,
            DEFAULT_LITERAL_LENGTH_NORMALIZED_COUNTS_LOG);
    private static final FseCompressionTable DEFAULT_OFFSETS_TABLE = FseCompressionTable.newInstance(
            DEFAULT_OFFSET_NORMALIZED_COUNTS, Constants.DEFAULT_MAX_OFFSET_CODE_SYMBOL,
            DEFAULT_OFFSET_NORMALIZED_COUNTS_LOG);

    private SequenceEncoder() {
    }

    public static int compressSequences(Object outputBase, final long outputAddress, int outputSize,
            SequenceStore sequences, CompressionParameters.Strategy strategy, SequenceEncodingContext workspace) {
        long output = outputAddress;
        long outputLimit = outputAddress + outputSize;

        Util.checkArgument(outputLimit - output > 3 /* max sequence count Size */ + 1 /* encoding type flags */,
        ...
        if (sequenceCount < 0x7F) {
            ...
            output++;
            UnsafeUtil.UNSAFE.putShort(outputBase, output,
                    (short) (sequenceCount - Constants.LONG_NUMBER_OF_SEQUENCES));
            output += Constants.SIZE_OF_SHORT;
        }
        ...
        int literalsLengthEncodingType = selectEncodingType(largestCount, sequenceCount,
                DEFAULT_LITERAL_LENGTH_NORMALIZED_COUNTS_LOG, true, strategy);
        ...
        // We can only use the basic table if max <= DEFAULT_MAX_OFFSET_CODE_SYMBOL,
        // otherwise the offsets are too large .
        boolean defaultAllowed = maxSymbol < Constants.DEFAULT_MAX_OFFSET_CODE_SYMBOL;

        int offsetEncodingType = selectEncodingType(largestCount, sequenceCount, DEFAULT_OFFSET_NORMALIZED_COUNTS_LOG,
                defaultAllowed, strategy);
        ...
        int matchLengthEncodingType = selectEncodingType(largestCount, sequenceCount,
                DEFAULT_MATCH_LENGTH_NORMALIZED_COUNTS_LOG, true, strategy);
        ...
        // flags
        UnsafeUtil.UNSAFE.putByte(outputBase, headerAddress,
                (byte) ((literalsLengthEncodingType << 6) | (offsetEncodingType << 4)
                        | (matchLengthEncodingType << 2)));

        output += encodeSequences(outputBase, output, outputLimit, matchLengthTable, offsetCodeTable,
                literalLengthTable, sequences);

        return (int) (output - outputAddress);
    }

    private static int buildCompressionTable(FseCompressionTable table, Object outputBase, long output,
            long outputLimit, int sequenceCount, int maxTableLog, byte[] codes, int[] counts, int maxSymbol,
            short[] normalizedCounts) {
        int tableLog = FiniteStateEntropy.optimalTableLog(maxTableLog, sequenceCount, maxSymbol);

        // this is a minor optimization. The last symbol is embedded in the initial FSE
        // state, so it's not part of the bitstream. We can omit it from the
        // statistics (but only if its count is > 1). This makes the statistics a tiny
        // bit more accurate.
        ...
        return FiniteStateEntropy.writeNormalizedCounts(outputBase, output, (int) (outputLimit - output),
                normalizedCounts, maxSymbol, tableLog); // TODO: pass outputLimit directly
    }
    ...
    private static int encodeSequences(
        ...
        blockStream.addBits(sequences.literalLengths[sequenceCount - 1],
                Constants.LITERALS_LENGTH_BITS[literalLengthCodes[sequenceCount - 1]]);
        blockStream.addBits(sequences.matchLengths[sequenceCount - 1],
                Constants.MATCH_LENGTH_BITS[matchLengthCodes[sequenceCount - 1]]);
        ...
        if (sequenceCount >= 2) {
            for (int n = sequenceCount - 2; n >= 0; n--) {
                ...
                if ((offsetBits + matchLengthBits + literalLengthBits >= 64 - 7
